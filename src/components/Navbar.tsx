...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header id="main-navbar" className="sticky top-0 left-0 right-0 z-50 transition-all duration-300">
      {/* Top Delivery Hotline Announcement Bar */}
      <div className="bg-gradient-to-r from-[#180903] via-amber-950 to-[#180903] text-amber-200 text-[10px] sm:text-xs py-1 px-3 sm:px-4 text-center border-b border-amber-800/40 flex items-center justify-between sm:justify-center gap-2">
      ...
      <div
        className={`transition-all duration-300 ${
          isScrolled
            ? 'bg-[#0a0503]/98 backdrop-blur-md border-b border-amber-900/50 shadow-2xl shadow-black/90 py-2 sm:py-2.5'
            : 'bg-[#0e0704]/90 backdrop-blur-sm border-b border-amber-950/60 py-2.5 sm:py-3.5'
        }`}
      >
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 flex items-center justify-between">
          {/* Brand Logo & Name */}
          <a href="#home" className="flex items-center gap-2.5 group shrink-0 mr-2 xl:mr-4">
            <Logo size="sm" showSubtitle={false} className="shrink-0" />
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                <span className="text-base sm:text-lg lg:text-xl font-extrabold tracking-wide text-amber-100 font-display group-hover:text-amber-300 transition-colors whitespace-nowrap">
                  CHAI MEHFIL
                </span>
              ...
              <span className="text-[10px] text-amber-400/80 font-medium tracking-wide hidden sm:block whitespace-nowrap">
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-2">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className={`text-xs xl:text-sm font-medium px-2.5 xl:px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 ${
                  link.isSpecial
                    ? 'text-amber-300 bg-amber-950/70 border border-amber-600/50 font-bold hover:bg-amber-600 hover:text-white'
                    : 'text-amber-100/85 hover:text-amber-300 hover:bg-amber-950/40'
                }`}
              >
                <link.icon className={`w-3.5 h-3.5 ${link.isSpecial ? 'text-amber-400' : 'text-amber-400/70'}`} />
                {link.label}
              </a>
            ))}
          </nav>
          ...
          <div className="hidden sm:flex items-center gap-2">
            {/* Language Toggle */}
            <div className="flex items-center bg-[#180d06] p-0.5 rounded-lg border border-amber-900/50 text-xs">
              <button
                id="lang-roman"
              ...
              <button
                ...
                className={`px-2 py-1 rounded font-urdu text-xs transition-all ${
                  currentLang === 'ur'
                    ? 'bg-amber-600 text-white shadow-sm'
                    : 'text-amber-200/70 hover:text-amber-100'
                }`}
            ...
            <button
              id="open-cart-btn"
              onClick={onOpenCart}
              className="relative p-2 rounded-lg bg-[#201007] text-amber-300 hover:bg-[#2e160a] border border-amber-800/40 transition-colors flex items-center gap-1.5 cursor-pointer shadow-md"
              title="Order Basket"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 text-white text-xs font-bold shadow-md shadow-amber-900/40 transition-all flex items-center gap-1 border border-amber-400/30 cursor-pointer whitespace-nowrap"
            >
